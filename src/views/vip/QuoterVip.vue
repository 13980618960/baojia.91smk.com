  ...
  <div class="vip-page">
    <el-card>
      ...
      <el-table :data="levels" border stripe>
        ...
        <el-table-column label="每日报价数">
          <template #default="{ row }">
            {{ row.daily_max_quotes }}条/天
          </template>
        </el-table-column>
    ...
    <el-dialog v-model="dialogVisible" :title="isEdit ? '编辑等级' : '新增等级'" width="500px">
      <el-form :model="form" label-width="100px">
        ...
        <el-form-item label="每日报价数">
          <el-input-number v-model="form.daily_max_quotes" :min="0" />
        </el-form-item>
...
const fetchList = async () => {
  try {
    const res = await request.get('/admin/vip/quoter')
...
const handleAdd = () => {
  isEdit.value = false
  Object.assign(form, { level_id: null, level_name: '', level_value: 1, daily_max_quotes: 0, priority_weight: 10, price: 0, sort: 0 })
...
const handleSubmit = async () => {
  try {
    if (isEdit.value) {
      await request.put(`/admin/vip/quoter/${form.level_id}`, form)
    } else {
      await request.post('/admin/vip/quoter', form)
    }
