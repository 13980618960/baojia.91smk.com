      ...
      <el-card shadow="hover" class="settings-card">
        <div class="global-settings">
          ...
          <div class="settings-content">
            ...
            <div class="setting-item">
              <span class="setting-label">每次轮播：</span>
              <el-input-number v-model="carouselCount" :min="1" :max="10" size="default" />
              <span class="setting-desc">条广告</span>
            </div>
            <div class="setting-item">
    ...
    <el-card class="content-card">
      ...
      <div class="ad-cards-grid" v-loading="loading">
        ...
        <div v-if="filteredList.length === 0 && !loading" class="empty-state">
          <el-empty description="暂无轮播图广告" />
        </div>
      </div>
    </el-card>

    <el-dialog v-model="dialogVisible" :title="isEdit ? '编辑轮播图广告' : '新增轮播图广告'" width="600px">
      <el-form :model="form" label-width="100px">
        ...
        <el-form-item label="图片URL" required>
          <ImageUpload
            ...
            upload-text="上传图片"
            tips="支持 JPG、PNG、GIF 格式，建议尺寸 1920x600"
          />
...
const fetchAdPricingRules = async () => {
  try {
    const res = await request.get('/pricing/ad-pricing/list', { params: { ad_type: 2 } })
...
const fetchList = async () => {
  loading.value = true
  try {
    const res = await request.get('/admin/ads/platform', { params: { position_id: 14 } })
    list.value = res.data || []
  } catch (error) {
    console.error(error)
  } finally {
...
const fetchGlobalSettings = async () => {
  try {
    const res = await request.get('/admin/ads/positions', { params: { code: 'banner_ad' } })
    if (res.data && res.data.length > 0) {
      globalIntervalType.value = res.data[0].interval_type || 'hour'
      globalIntervalValue.value = res.data[0].interval_value || 5
      carouselCount.value = res.data[0].carousel_count || 3
    }
  } catch (error) {
    console.error(error)
  }
...
const handleSaveGlobalSettings = async () => {
  saving.value = true
  try {
    await request.put('/admin/ads/positions/code/banner_ad', {
      interval_type: globalIntervalType.value,
      interval_value: globalIntervalValue.value,
      carousel_count: carouselCount.value
    })
