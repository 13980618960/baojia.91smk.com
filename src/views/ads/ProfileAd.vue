  ...
  <div class="profile-ad">
    ...
    <el-card class="content-card">
      ...
      <el-tabs v-model="activeTab" @tab-change="handleTabChange">
        <el-tab-pane label="需求方个人中心" name="demander" />
        <el-tab-pane label="报价方个人中心" name="quoter" />
      </el-tabs>
      
      <div class="ad-cards-grid" v-loading="loading">
        <el-card v-for="ad in filteredList" :key="ad.ad_id" class="ad-card-item" shadow="hover" :data-position="ad.position">
          <div class="ad-card-content">
            ...
            <div class="ad-info">
              ...
              <p class="ad-content" v-if="ad.content">{{ ad.content }}</p>
              <div class="ad-meta">
                <div class="meta-item">
        ...
        <div v-if="filteredList.length === 0 && !loading" class="empty-state">
          <el-empty :description="activeTab === 'demander' ? '暂无需求方个人中心广告' : '暂无报价方个人中心广告'" />
        </div>
      </div>
    </el-card>

    <el-dialog v-model="dialogVisible" :title="isEdit ? '编辑个人中心广告' : '新增个人中心广告'" width="600px">
      <el-form :model="form" label-width="100px">
        <el-form-item label="广告位置" required>
          <el-select v-model="form.position" placeholder="请选择广告位置">
            <el-option label="需求方个人中心" value="demander_profile" />
            <el-option label="报价方个人中心" value="quoter_profile" />
          </el-select>
        </el-form-item>
        ...
        <el-form-item label="图片URL" required>
          <ImageUpload
            ...
            upload-text="上传图片"
            tips="支持 JPG、PNG、GIF 格式，建议尺寸 800x800"
          />
            ...
            <el-tabs v-model="editorTab" type="border-card">
              <el-tab-pane label="编辑" name="edit">
                ...
                <div 
                  ...
                  @paste="handlePaste"
                  placeholder="请输入图文广告内容描述，支持粘贴图片"
                ></div>
              </el-tab-pane>
...
const uploadImageFile = (file) => {
  ...
  .then(response => {
    if (response && response.code === 200 && response.data && response.data.length > 0 && response.data[0].url) {
      ...
      execCommand('insertHTML', `<img src="${fullImageUrl}" alt="" style="max-width: 100%; height: auto;" />`)
      ElMessage.success('图片上传成功')
    } else {
      ElMessage.error('图片上传失败')
    }
...
const insertDivider = () => {
  execCommand('insertHTML', '<hr style="border: 1px solid #e0e0e0; margin: 10px 0;" />')
  form.content = contentEditor.value.innerHTML
}
...
const filteredList = computed(() => {
  ...
  const positionMap = {
    demander: 'demander_profile',
    quoter: 'quoter_profile'
  }
  result = result.filter(item => item.position === positionMap[activeTab.value])
...
const fetchAdPricingRules = async () => {
  try {
    const res = await request.get('/pricing/ad-pricing/list', { params: { ad_type: 5 } })
...
const fetchList = async () => {
  loading.value = true
  try {
    const positionId = activeTab.value === 'demander' ? 6 : 7
    const res = await request.get('/admin/ads/platform', { params: { position_id: positionId } })
    list.value = res.data || []
  } catch (error) {
    console.error(error)
  } finally {
...
const fetchGlobalSettings = async () => {
  try {
    const res = await request.get('/admin/ads/positions', { params: { code: 'profile_ad' } })
...
const handleSaveGlobalSettings = async () => {
  saving.value = true
  try {
    await request.put('/admin/ads/positions/code/profile_ad', {
...
const handleAdd = () => {
  ...
  Object.assign(form, {
    ...
    title: '',
    type: 4,
    position: activeTab.value === 'demander' ? 'demander_profile' : 'quoter_profile',
    billing_type: 'cpm',
    rule_id: '',
    fund_account_id: '',
    image_url: '',
...
const handleEdit = async (row) => {
  ...
  // 根据当前标签页设置 position，确保编辑后保存的广告仍然显示在当前标签页
  const currentPosition = row.position || (activeTab.value === 'demander' ? 'demander_profile' : 'quoter_profile')
  form.position = currentPosition
...
const resetFormData = () => {
  ...
  form.weight = 50
  form.status = 1
  form.position = 'demander_profile'
  dateRange.value = []
  editCities.value = []
  editDistricts.value = []
}
...
const handleSubmit = async () => {
  try {
    ...
    let positionId = 6
    if (form.position === 'demander_profile') {
      positionId = 6
    } else if (form.position === 'quoter_profile') {
      positionId = 7
    }
    ...
    if (isEdit.value) {
      await request.put(`/admin/ads/platform/${form.ad_id}`, submitData)
    } else {
...
/* 根据广告类型设置不同的图片比例 - 1920x400 */
.ad-card-item[data-position="demander_profile"] .ad-image {
  aspect-ratio: 1920/400;
}

.ad-card-item[data-position="quoter_profile"] .ad-image {
  aspect-ratio: 1920/400;
}
