    ...
    <el-card class="content-card">
      ...
      <div class="ad-cards-grid" v-loading="loading">
        ...
        <div v-if="filteredList.length === 0 && !loading" class="empty-state">
          <el-empty description="暂无需求卡片广告" />
        </div>
      </div>
    </el-card>

    <el-dialog v-model="dialogVisible" :title="isEdit ? '编辑需求卡片广告' : '新增需求卡片广告'" width="600px">
      <el-form :model="form" label-width="100px">
        ...
        <el-form-item label="图片URL" required>
          <ImageUpload
            ...
            upload-text="上传图片"
            tips="支持 JPG、PNG、GIF 格式，建议尺寸 800x400"
          />
...
const fetchAdPricingRules = async () => {
  try {
    const res = await request.get('/pricing/ad-pricing/list', { params: { ad_type: 3 } })
...
const fetchList = async () => {
  loading.value = true
  try {
    const res = await request.get('/admin/ads/platform', { params: { position_id: 4 } })
    list.value = res.data || []
  } catch (error) {
    console.error(error)
  } finally {
...
const fetchGlobalSettings = async () => {
  try {
    const res = await request.get('/admin/ads/positions', { params: { code: 'demand_hall_feed' } })
...
const handleSaveGlobalSettings = async () => {
  saving.value = true
  try {
    await request.put('/admin/ads/positions/code/demand_hall_feed', {
...
const handleEdit = async (row) => {
  ...
  form.position = row.position || 'demand_hall_feed'
...
.color-preview, .bg-color-preview {
