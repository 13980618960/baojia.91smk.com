    ...
    <el-card class="content-card">
      ...
      <div class="ad-cards-grid" v-loading="loading">
        <el-card v-for="ad in filteredList" :key="ad.ad_id" class="ad-card-item" shadow="hover">
          <div class="ad-card-content">
            ...
            <div class="ad-info">
              ...
              <div class="ad-video" v-if="ad.video_url">
                <el-link type="primary" :href="ad.video_url" target="_blank">查看视频</el-link>
              </div>
              <div class="ad-meta">
                <div class="meta-item">
        ...
        <div v-if="filteredList.length === 0 && !loading" class="empty-state">
          <el-empty description="暂无开屏广告" />
        </div>
      </div>
    </el-card>

    <el-dialog v-model="dialogVisible" :title="isEdit ? '编辑开屏广告' : '新增开屏广告'" width="600px">
      <el-form :model="form" label-width="100px">
        ...
        <el-form-item label="封面图片" required>
          <ImageUpload
            ...
            upload-text="上传图片"
            tips="支持 JPG、PNG、GIF 格式，建议尺寸 1080x1920"
          />
...
const fetchAdPricingRules = async () => {
  try {
    const res = await request.get('/pricing/ad-pricing/list', { params: { ad_type: 1 } })
...
const fetchList = async () => {
  loading.value = true
  try {
    const res = await request.get('/admin/ads/platform', { params: { position_id: 13 } })
    list.value = res.data || []
  } catch (error) {
    console.error(error)
  } finally {
...
const fetchGlobalSettings = async () => {
  try {
    const res = await request.get('/admin/ads/positions', { params: { code: 'splash_ad' } })
...
const handleSaveGlobalSettings = async () => {
  saving.value = true
  try {
    await request.put('/admin/ads/positions/code/splash_ad', {
